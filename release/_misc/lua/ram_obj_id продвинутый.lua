...
local tbl = {
    ...
    [0x01] = nil,  -- капсула
    [0x02] = nil,  -- пуля большинства мобов и пушек
...
while true do
    ...
    while true do
        ...
        if previous_status[index] == nil then
            previous_status[index] = {}
            previous_status[index][id] = nil
        end
        
        if id == 0x7F then
            previous_status[index] = nil
        else
            local X = memory.readbyte(pos_X_hi_obj + index)
            local Y = memory.readbyte(pos_Y_hi_obj + index)
            
            if tbl[id] == nil then
                id = string.format("%02X", id)
                gui.text(X, Y, id..":", "white", nil)
                
                local stat = memory.readbyte(ram_obj_status + index)
                if previous_status[index][id] == stat then
                    stat = string.format("%01X", stat)
                    gui.text(X + 16, Y, stat, "white", nil)
                else
                    previous_status[index][id] = stat
                    stat = string.format("%01X", stat)
                    gui.text(X + 16, Y, stat, "red", "white")
                end
            end
        end
        index = index + 1
