...
.export sub_0x01F4B5
.export sub_0x01F4BB
.export tbl_0x01F4E4_угол_поворота_объекта
.export sub_0x01F4FC
.export sub_0x01F500
...
C - - - - - 0x01F4A5 07:F495: 20 A2 F4  JSR sub_F4A3
loc_F498:
C - - - - - 0x01F4A8 07:F498: BC 48 07  LDY ram_счетчик_поворота_объекта,X
sub_0x01F4AB:
C - - - - - 0x01F4AB 07:F49B: B9 D4 F4  LDA tbl_F4D4_угол_поворота_объекта,Y
C - - - - - 0x01F4AE 07:F49E: 9D 56 07  STA ram_угол_направления_объекта,X
C - - - - - 0x01F4B1 07:F4A1: 60        RTS

tbl_F4D4_угол_поворота_объекта:
tbl_0x01F4E4_угол_поворота_объекта:
...
sub_F4A3:
                                        ...
                                        JSR sub_F370_расчет_угла_от_объекта_до_игрока
                                        ...
                                        LSR
; 00
                                        LDA #$06    ; 180 градусов? противоположная сторона
                                        LDY #$0C    ; максимальный счетчик поворота
                                        BCC bra_F517
sub_F4F1:
; 01
                                        LDA #$0C    ; 180 градусов? противоположная сторона
                                        LDY #$18    ; максимальный счетчик поворота, предел
bra_F517:
                                        STA ram_0005    ; минимальный счетчик поворота
                                        STY ram_0004    ; максимальный счетчик поворота

                                        LDA #$00
                                        STA ram_000E
                                        LDA ram_счетчик_поворота_объекта,X  ; текущий угол поворота
                                        CLC
                                        ADC ram_0005    ; минимальный счетчик поворота
                                        CMP ram_0004    ; максимальный счетчик поворота
                                        BCC bra_F54C
                                        INC ram_000E    ; был перебор через максимум
                                        SBC ram_0004    ; максимальный счетчик поворота
bra_F54C:
                                        STA ram_000D    ; повернули в противоположную сторону

                                        LDA ram_000C    ; разница угол от объекта до игрока
...
sub_0x01F4BB:
C - - - - - 0x01F4BB 07:F4AB: 20 AE AA  JSR sub_0x006ABE_копировать_позиции_спрайта_объекта
C - - - - - 0x01F4BE 07:F4AE: 20 EC F4  JSR sub_F4EC
loc_F4B1:   ; ram_000F = 00-01
...
C - - - - - 0x01F5E0 07:F5D0: A5 0A     LDA ram_000A    ; позиция_y_спрайта_игрока
